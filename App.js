...
function Studio() {
  ...
  const history = useHistory(clonePattern(DEFAULT_PATTERN)); const pattern = history.state;
  const [sequenceLength, setSequenceLength] = useState(16); const [swing, setSwing] = useState(0); const [mutes, setMutes] = useState({});
  const [trackVolumes, setTrackVolumes] = useState(DEFAULT_TRACK_VOLUMES); const [volume, setVolume] = useState(0.85); const [rate, setRate] = useState(1);
  const [activeStems, setActiveStems] = useState({}); const [lpf, setLpf] = useState(false); const [echo, setEcho] = useState(false);
  const [projectId, setProjectId] = useState(null); const [projectName, setProjectName] = useState('Unsaved Beat'); const [hydrated, setHydrated] = useState(false);
  const patternRef = useRef(pattern); patternRef.current = pattern; const mutesRef = useRef(mutes); mutesRef.current = mutes; const readyRef = useRef(ready); readyRef.current = ready;
  const onStepFired = useCallback((step) => { if (!readyRef.current || step >= sequenceLength) return; const pat = patternRef.current, mut = mutesRef.current; SEQ_TRACKS.forEach((track) => { if (!mut[track.id] && pat[track.id][step]) trigger(track.padId, step % 4 === 0 ? 1 : 0.82); }); }, [trigger, sequenceLength]);
  ...
  const handleTrackVolume = useCallback((trackId, value) => { setTrackVolumes((prev) => ({ ...prev, [trackId]: value })); const track = SEQ_TRACKS.find((t) => t.id === trackId); if (track) engine.current?.setGroupVolume(track.group, value); }, [engine]);
  const toggleStem = useCallback((id) => setActiveStems((prev) => ({ ...prev, [id]: !prev[id] })), []); const toggleLpf = useCallback(() => setLpf((prev) => !prev), []);
  ...
  const clearPattern = useCallback(() => history.setState(emptyPattern()), [history.setState]);
  const randomPattern = useCallback(() => { const density = { kick: 0.28, snare: 0.16, hat: 0.55, synth: 0.14 }; history.setState(Object.fromEntries(SEQ_TRACKS.map((t) => [t.id, Array.from({ length: STEPS }, (_, i) => { if (t.id === 'kick' && i === 0) return 1; if (t.id === 'snare' && (i === 4 || i === 12)) return 1; return Math.random() < density[t.id] ? 1 : 0; })]))); }, [history.setState]);
  ...
  return <View style={[styles.root, { paddingTop: insets.top }]}><StatusBar barStyle="light-content" backgroundColor={C.bg} /><View style={styles.header}><View style={styles.brandRow}><View style={styles.brandIcon}><AudioWaveform size={17} color={C.cyan} /></View><View><Text style={styles.brand}>BEATFORGE</Text><Text style={styles.brandSub}>{playing ? '● LIVE' : '○ IDLE'} · {stemsOn} STEM{stemsOn === 1 ? '' : 'S'} · {rate.toFixed(2)}x</Text></View></View><View style={[styles.syncPill, playing && { borderColor: C.lime }]}><Text style={[styles.syncText, playing && { color: C.lime }]}>SYNC {STEM_BPM}</Text></View></View><ScrollView style={styles.scroll} contentContainerStyle={[styles.scrollContent, { paddingBottom: insets.bottom + SPACE.xl }]} showsVerticalScrollIndicator={false}>
    <ProjectBar projectName={projectName} onSave={handleSave} onNew={newProject} onLoad={handleLoad} /><Transport playing={playing} onToggle={toggle} bpm={bpm} onTapTempo={tapTempo} tapCount={tapCount} onClear={clearPattern} onRandom={randomPattern} /><HistoryBar onUndo={history.undo} onRedo={history.redo} canUndo={history.canUndo} canRedo={history.canRedo} />
    ...
    <Section icon={<Disc3 size={12} color={C.textFaint} />} title="LOOP STEMS · BEAT-SYNCED"><StemBar activeStems={activeStems} onToggle={toggleStem} /></Section><Section icon={<AudioWaveform size={12} color={C.textFaint} />} title="FILTER FX"><FxBar lpf={lpf} onLpf={toggleLpf} echo={echo} onEcho={() => setEcho((p) => !p)} onRiser={fireRiser} onVinyl={fireVinyl} /></Section>
    <Text style={styles.footer}>Projects auto-save locally · undo/redo · 16 pads · 4 tracks · 16/32/64 steps · swing · per-track mix</Text></ScrollView></View>;
}
