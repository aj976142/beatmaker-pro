...
export default class SoundEngine {
  ...
  async loadAll(onProgress = () => {}) {
    ...
    for (const pad of PADS) { const voices = []; for (let i = 0; i < (POOL_SIZE[pad.id] ?? POOL_SIZE.default); i += 1) { const { sound } = await Audio.Sound.createAsync(pad.src, { shouldPlay: false, volume: this.masterVolume * (this.groupVolumes[pad.group] ?? 1), rate: this.rate, shouldCorrectPitch: false }); voices.push(sound); this.allSounds.push(sound); } this.pools[pad.id] = { voices, idx: 0, group: pad.group || 'fx' }; bump(pad.label); }
    for (const [id, src] of Object.entries(EXTRA)) { const { sound } = await Audio.Sound.createAsync(src, { shouldPlay: false, volume: this.masterVolume, rate: this.rate, shouldCorrectPitch: false }); this.pools[id] = { voices: [sound], idx: 0, group: 'fx' }; this.allSounds.push(sound); bump(id); }
